...
export default function ArticleDetail() {
  ...
  useEffect(() => {
    const foundArticle = (articlesData as ArticleType[]).find(a => a._id === id);
    if (!foundArticle) {
      notFound();
    }
  ...
  if (article.tags?.length) {
    const sameTagArticles = (articlesData as ArticleType[]).filter(
      a => a._id !== article._id && a.tags?.some(tag => article.tags?.includes(tag))
    );

    if (sameTagArticles.length < 3) {
      const otherArticles = (articlesData as ArticleType[]).filter(
